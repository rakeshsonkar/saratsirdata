...
public class EmployeeServiceImpl implements EmployeeService {
	...
	public EmployeeDTO updateEmployeeInfo(EmployeeDTO employeeDTO) {
		...
		Employee employee = new Employee();
		/*
		 * the following block gets executed when passed UPDATE as type of operation to
		 * be performed with the given employee details
		 */
		if (employeeDTO != null && employeeDTO.getType() != null
				&& employeeDTO.getType().equalsIgnoreCase(ActionType.UPDATE.getType())) {
			LOGGER.info("Updating Employee with the details :: " + employeeDTO.toString());
			employee.setEmail(employeeDTO.getEmail());
			employee.setEmployeeName(employeeDTO.getEmployeeName());
			if (Objects.nonNull(employeeDTO.getAddressDTOList()) && employeeDTO.getAddressDTOList().size() != 0) {
				List<Address> addressList = new ArrayList<>();
			...
			try {
				// update employee from repository
				employeeRepository.update(employee, SQLQuery.UPDATE_EMPLOYEE_BY_ID.getQueryString());
			} catch (IfmsBaseException ifmsBaseException) {
				LOGGER.error("Employee with the employeeId :: [" + employeeDTO.getEmployeeId() + "] could not be Updated" + ifmsBaseException.getMessage());
			}
		...
		employeeDTO.getIfmsResponse().setResponseCode(IFMSResponseCode.SUCCESS.getCode());
		employeeDTO.getIfmsResponse().setResponseMessage(IFMSResponseCode.SUCCESS.getMessage());
		LOGGER.info("Employee with the employeeId :: [" + employeeDTO.getEmployeeId() + "] Updated");
		return employeeDTO;
	}
